...
class HEMSAvailability():
    ...
    def preferred_resource_available(self, pt: Patient, preferred_care_category: str, helicopter_benefit: str) -> list[HEMS | None, int, bool]:
        """
        ...
        for h in self.store.items:
            ...
            # REG = if helicopter benefit then H71 then H70 otherwise free for all.
            ...
            if not h.in_use and h.hems_resource_on_shift(pt.hour, pt.qtr):
                ...
                if preferred_care_category in ['CC', 'EC']:
                    ...
                    if h.category == preferred_care_category and h.vehicle_type == "helicopter" and not h.being_serviced:
                        ...
                        if h.vehicle_type == 'helicopter' and not h.being_serviced:
                            hems = h
                        ...
                        if h.vehicle_type == 'helicopter' and not h.being_serviced:
                            hems = h
                            preferred = 5 # Third  choice (EC helicopter)
    ...
    def allocate_resource(self, pt: Patient) -> Any | Event:
        ...
        print(f"Attempting to allocate resource with care category {pt.hems_cc_or_ec} and preferred helicopter benefit {pt.hems_helicopter_benefit}")
